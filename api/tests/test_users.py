...
async def test_verify_token_existence(async_client: AsyncClient):
    ...
    response = await async_client.delete("/users/token/" ,params={'token_to_delete':token}, headers={"token":token})
    assert response.status_code == 200
    response = await async_client.get(
        "/users/", headers={"token":token}
    )
...
async def test_delete_token(async_client: AsyncClient):
    ...
    assert response.status_code == 200
    token = response.json()['access_token']
    response = await async_client.delete("/users/token/", params={"token_to_delete":token})
...
async def test_update_user(async_client: AsyncClient):
    response = await async_client.put(
        "/users/",
        json={"password": "newpassword", "categories": ["Health", "Travel"], "currencies":["INR"]}
    )
    assert response.status_code == 200
